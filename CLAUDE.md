...
useMenuGeneration (hook)
  → geminiService.generateWeeklyMenu()   ← Edge Function gemini-proxy → Gemini (JSON mode; model chosen server-side)
...
**Kitchen equipment exclusions**: `batchfit:equipment` (`EquipmentPrefs`, edited in Generar → "Equipamiento de cocina") lists appliances the user can't or won't use (broken oven, no air fryer…). The catalog and helpers live in `src/utils/equipment.ts`; excluded appliances are injected into every Gemini prompt (weekly menu, single-meal swap, batch guide) via `buildEquipmentSection()` and filter the base-recipe fallback by keyword-matching recipe steps. Because the model sometimes ignores the instruction, AI output is also **validated** (`validateMenuResponse`/`validateGuideResponse` in `src/utils/validators.ts` scan recipe steps and guide tasks with `findExcludedApplianceMentions`); failures are fed back to Gemini on retry and, as a last resort, `stripExcludedEquipment()` marks offending tasks as "⚠ Adaptar" instead of leaving the instruction intact.

**Two-slot menu model**: besides the active menu (`batchfit:current_menu` + shopping list + batch guide), a menu for the **next week** can be generated in advance into `batchfit:next_menu` / `batchfit:next_shopping_list` / `batchfit:next_batch_guide` without touching the current one (Generar has an "Esta semana / Próxima semana" toggle; Compra and Mi Menú have week switchers — Compra defaults to the current list while it still has pending items, then to the next one). `WeeklyMenu.weekStart` (Monday of the consumption week) makes the shared date logic (`getWeekStart`) work for future weeks; old menus fall back to the `generatedAt` heuristic. The single source of truth for "which week is *this* week" is `upcomingWeekStartISO()` in `src/utils/dateUtils.ts` (Mon–Fri → this Monday; Sat/Sun → next Monday); the current slot must always hold that week and the next slot the following one. `src/services/promotionService.ts` auto-promotes next→current **on Saturday** (as soon as the previous consumption week ends and the planned week becomes the week in focus; wired in `App.tsx`: mount, `visibilitychange`, remote updates, and at the start of every generation), and `send-reminders` reads both slots and picks whichever menu is active today (`isMenuActiveOn`: Saturday..Friday). Promotion never deletes data: if both slots hold the same week (legacy state), Generar shows a collision banner with an "Activar el planificado" button, and regenerating "Esta semana" is the only thing that clears a superseded next slot.
...
| `supabase/functions/gemini-proxy/` | Authenticated proxy adding the server-side `GEMINI_API_KEY`; enforces `ALLOWED_EMAILS`; picks the model (`GEMINI_MODEL`, default `gemini-3.5-flash`) with a fallback chain (`GEMINI_FALLBACK_MODELS`) on 404/429/503 and reports it in the `x-batchfit-model` response header |
...
```
Gemini/YouTube API keys live as Edge Function secrets (`GEMINI_API_KEY`, `YOUTUBE_API_KEY`, `YOUTUBE_PLAYLIST_ID`, `ALLOWED_EMAILS`; optional `GEMINI_MODEL` / `GEMINI_FALLBACK_MODELS` to switch models without a frontend deploy); push reminders add `VAPID_KEYS` + `VAPID_CONTACT` (function secrets) and `project_url`/`service_role_key` in Vault for pg_cron — see `SETUP-SUPABASE.md`. Without the `VITE_SUPABASE_*` vars the app runs in local mode (no login, base recipes, Videos tab shows a setup message). The GitHub Pages deploy workflow injects them from repo Actions secrets.
